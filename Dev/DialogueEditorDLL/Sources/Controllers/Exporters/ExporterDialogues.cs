...
{
    public static class ExporterDialogues
    {
        //--------------------------------------------------------------------------------------------------------------
        // Exports
        ...
        public static void ExportConstantsToCsv(string directory, Project project, List<Language> languages)
        {
            //Ensure no export of a Workstring language
        ...
        public static void ExportDialoguesToCsv(string directory, Project project, List<Dialogue> dialogues, List<Language> languages, DateTime dateFrom, ExporterStats.ProjectStats projectStats)
        {
            //Ensure no export of a Workstring language
            //languages.Remove(EditorCore.LanguageWorkstring);
        ...
        public static bool ImportFromCsv()
        {
            ...
            if (dialog.ImportConstants)
            {
                ...
                if (resultConstants)
                    ProjectController.LogInfo("Import Constants Finished");
                else
                    ProjectController.LogError("Import Constants Failed");
            }
            ...
            if (dialog.ImportLocalization || dialog.ImportWorkstring || dialog.ImportInformation)
            {
                ...
                if (resultDialogues)
                    ProjectController.LogInfo("Import Dialogues Finished");
                else
                    ProjectController.LogError("Import Dialogues Failed");
            }

            return resultConstants || resultDialogues;
        }

        public static bool ImportConstantsFromCsv(string importPath, List<Language> languages, bool importLocalization, bool importWorkstring, bool importInformation)
        {
            var project = ProjectController.Project;
            ...
            using (StreamReader file = new StreamReader(importPath, Encoding.UTF8))
            {
                        ...
                        if (constant != null)
                        {
                            ...
                            if (importWorkstring)
                            {
                                ...
                                if (constant.LastEditDate > timestampLoca)
                                {
                                    ...
                                    ProjectController.LogWarning(string.Format("Constant {0} - New workstring older than currently registered workstring, but updated anyway", id));
                                }
                            ...
                            if (importLocalization)
                            {
                                foreach (var language in languages)
                                {
                                        ...
                                        || translationResult == ETranslationResult.Accepted_IdenticalText)
                                    {
                                        ...
                                        {
                                            ProjectController.LogWarning(string.Format("Constant {0} - [{1}] Translation accepted but based on an outdated workstring, will be re-exported", id, language.Name));
                                        }
                                        else
                                        {
                                    ...
                                    {
                                        ProjectController.LogWarning(string.Format("Constant {0} - [{1}] Translation accepted but with an identical timestamp as the previous entry", id, language.Name));
                                    }
                                    else if (translationResult == ETranslationResult.Accepted_IdenticalText)
                                    {
                                        ProjectController.LogWarning(string.Format("Constant {0} - [{1}] Translation accepted but with an identical text as the previous entry", id, language.Name));
                                    }
        ...
        public static bool ImportDialoguesFromCsv(string importPath, List<Package> packages, List<Language> languages, bool importLocalization, bool importWorkstring, bool importInformation)
        {
            var project = ProjectController.Project;
            ...
            using (StreamReader file = new StreamReader(importPath, Encoding.UTF8))
            {
                ...
                if (reader.ParseHeaders(file, headerRedirects))
                {
                    while (reader.ParseNextLine())
                    {
                        Dialogue dialogue = ProjectController.GetDialogue(reader.GetCell("Dialogue"));
                        if (dialogue != null)
                        {
                            ...
                            else
                            {
                                ...
                                if (node is DialogueNodeSentence)
                                {
                                    ...
                                    if (importInformation)
                                    {
                                        ...
                                        string speakerID = ProjectController.Project.GetActorID(reader.GetCell("Speaker"));
